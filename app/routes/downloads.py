...
def start_download():
    if not vcfdt_service.has_active_token():
        flash('No activation code or token found. Please configure one first.', 'error')
        return redirect(url_for('downloads.downloads_page'))

    data = request.form
    try:
        job_id = vcfdt_service.start_download_job(
        ...
        flash(f'Download failed to start: {str(e)}', 'error')
